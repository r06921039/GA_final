...
int main (int argc, char *argv[])
{
    ...
    course_parse("course_sub.txt", c);
    room_parse("room_sub.txt", r);
    classroom_num = r.size();

    ell = c.size();
    
    for (i = 0; i < repeat; i++) {

        GA ga (ell, nInitial, selectionPressure, pc, pm, maxGen, maxFe, r, c);
        ...
        Chromosome ch(ell, classroom_num);
